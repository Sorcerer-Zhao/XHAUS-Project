...
function updateRuntime(runtime) {
  ...
  renderWsHistory();
  updateActiveAgent(runtime.active_agent);
...
async function loadCognitionDocs() {
  try {
    ...
    $("#cogList").innerHTML = docs.length
      ? docs
          .slice(0, 4)
          .map((doc) => `<div class="doc-item">${escapeHtml(doc.name)}</div>`)
          .join("")
      : '<div class="doc-item">暂无文档</div>';
  } catch (err) {
    $("#cogList").innerHTML = '<div class="doc-item">文档列表不可用</div>';
  }
}
...
async function launchApp() {
  ...
  await Promise.allSettled([ensureSession(), fetchPresetChoices(), loadCognitionDocs(), loadSatelliteStatus()]);
