...
function wsToHttpChatUrl(value) {
  const raw = normalizeWsUrl(value);
  if (!raw) {
    return "";
  }
  try {
...
function setRuntimeGatewayUrl(value) {
  const url = normalizeWsUrl(value);
  if (/^wss?:\/\//i.test(url)) {
    runtimeGatewayUrl = url;
  }
}

function resolveChatCompletionsUrl() {
  return wsToHttpChatUrl(runtimeGatewayUrl) || String(OPENCLAW_CHAT_COMPLETIONS_URL || "").trim();
}
...
function resolveGateway() {
  ...
  let url = runtimeGatewayUrl || normalizeWsUrl(configuredUrl);
