...
function readableOpenClawError(err, agentId) {
  const detail = err && err.message ? err.message : "OpenClaw 暂时不可用";
  if (/Unknown agent id|404:\s*Not Found|HTTP 404/i.test(detail)) {
    return `当前管家 ${agentId} 还没有在 OpenClaw Gateway 完成注册。请等右侧 Runtime 显示 XHAUS 已连接/已激活后再发送；如果刚刚创建了新管家，请等待创建流程结束，必要时重启 Gateway。`;
  }
  return detail;
}
...
router.post("/chat", async (req, res) => {
  ...
  openclawClient
    ...
    .catch((err) => {
      const detail = readableOpenClawError(err, agentId);
