...
提案驱动路径 (新增)
------------------
  proposal -> compile_instruction_from_proposal() -> generate_skill_from_proposal()
  -> 复用 scaffold / generate_executor / validate 质量门.

纯文本路径 (保留)
-----------------
  generate_skill_to_staging(target_name, compile_instruction, ...)
...
# 复用阶段二的"项目通用 LLM API" 与网关结构化草案
from gateway_router import (  # noqa: E402
    GatewayDecision,
    LLMClient,
    NewSkillSpec,
    OpenAIClient,
    is_skill_execution_ready,
)
...
PROPOSAL_FILE = "skill_proposal.json"
...
@dataclass
class SkillProposal:
    """提案驱动的 Skill 规格 — 网关 opportunity 层落地前的结构化输入."""
    ...
    def from_dict(cls, data: dict[str, Any]) -> "SkillProposal":
        ...
        return cls(
            ...
            trigger_condition=str(data.get("trigger_condition") or "").strip(),
            inputs=_list("inputs"),
...
def _make_description(
    ...
    *,
    ...
    """合成 SKILL.md 一行描述; 优先 proposal.purpose."""
    if proposal and proposal.purpose.strip():
        text = proposal.purpose.strip()
    else:
    ...
    return _sanitize_description_text(text)
...
def _write_proposal(skill_dir: Path, proposal: SkillProposal) -> Path:
    """把结构化提案落盘, 供 HIL 审阅与后续迭代."""
    path = skill_dir / PROPOSAL_FILE
    payload = {
        "proposal": proposal.to_dict(),
        "staged_at": datetime.now(timezone.utc).isoformat(),
    }
    path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    logger.info("Wrote %s -> %s", PROPOSAL_FILE, path)
    return path
...
def compile_instruction_from_proposal(
    proposal: SkillProposal,
    *,
    draft_mode: bool = False,
) -> str:
    """
    提案 -> compile_instruction 转换层.

    draft_mode=True 时用于 new_skill_proposal: 标注为最小草案,
    引导 Skill Compiler 在信息不足处做保守假设.
    """
    name = proposal.normalized_name() or proposal.name.strip()
    lines: list[str] = [f"# Skill: {name}"]
    if draft_mode:
        lines.append(
            "[MINIMAL DRAFT] Expand conservatively where fields are sparse; "
            "do not invent tools not listed in required_tools."
        )
    if proposal.purpose:
        lines.append(f"Purpose: {proposal.purpose}")
    if proposal.trigger_condition:
        lines.append(f"Trigger: {proposal.trigger_condition}")
    lines.append("")
    lines.extend(_bullet_section("## Inputs", proposal.inputs))
    lines.append("")
    lines.extend(_bullet_section("## Outputs", proposal.outputs))
    lines.append("")
    lines.extend(_bullet_section("## Required tools", proposal.required_tools))
    lines.append("")
    lines.extend(_bullet_section("## Steps", proposal.steps))
    lines.append("")
    lines.extend(_bullet_section("## Success criteria", proposal.success_criteria))
    lines.append("")
    lines.extend(_bullet_section("## Failure modes", proposal.failure_modes))
    lines.append("")
    lines.append(
        "Implement a single entrypoint run(...) matching inputs/outputs; "
        "handle listed failure modes explicitly."
    )
    return "\n".join(lines).strip()


def expand_minimal_proposal(proposal: SkillProposal) -> SkillProposal:
    """new_skill_proposal 路径: 补齐缺失字段, 形成最小可执行草案."""
    name = proposal.normalized_name() or proposal.name
    purpose = proposal.purpose.strip() or f"Automate workflow for {name}"
    trigger = proposal.trigger_condition.strip() or (
        f"When user intent matches: {purpose}"
    )
    tools = list(proposal.required_tools)
    steps = list(proposal.steps)
    if not steps and tools:
        steps = [f"{i + 1}) Invoke {tool}(...)" for i, tool in enumerate(tools)]
    elif not steps:
        steps = ["1) Parse user input", "2) Execute required_tools in order", "3) Return structured output"]

    inputs = list(proposal.inputs) or ["Primary user input (string or structured dict)"]
    outputs = list(proposal.outputs) or ["Structured result from the tool workflow"]
    success = list(proposal.success_criteria) or [
        "All required_tools complete without unhandled exceptions",
        "Output matches the declared outputs schema",
    ]
    failures = list(proposal.failure_modes) or [
        "Tool timeout or network error",
        "Empty or invalid input",
        "Partial tool chain failure — return error detail, do not crash",
    ]
...
def proposal_from_new_skill_spec(
    spec: NewSkillSpec,
    *,
    name: str = "",
    purpose: str = "",
    tool_chain_signature: str = "",
) -> SkillProposal:
    """从 gateway_router.NewSkillSpec 构建 SkillProposal."""
    resolved_name = name.strip() or spec.name.strip()
    resolved_purpose = purpose.strip() or spec.description.strip() or spec.rationale.strip()
    triggers = ", ".join(t for t in spec.proposed_triggers if t.strip())
    tools = list(spec.tool_chain)
    steps: list[str] = []
    if tools:
        steps = [f"{i + 1}) Call {tool}(...)" for i, tool in enumerate(tools)]
    elif tool_chain_signature:
        steps = [f"Execute stable chain: {tool_chain_signature}"]

    return SkillProposal(
        name=resolved_name,
        purpose=resolved_purpose,
        trigger_condition=triggers or resolved_purpose,
        inputs=["User-provided parameters for the workflow"],
        outputs=["Workflow result (structure per purpose)"],
        required_tools=tools,
        steps=steps,
        success_criteria=[
            "Tool chain completes in order",
            "Returns output matching purpose",
        ],
        failure_modes=[
            "Any tool in required_tools fails",
...
def proposal_from_gateway_decision(decision: GatewayDecision) -> SkillProposal:
    """从 GatewayDecision 提取/合成 SkillProposal (tool_chain / new_skill_proposal)."""
    sg = decision.skill_generation
    opp = decision.skill_opportunity
    spec = (opp.new_skill_spec if opp else None) or sg.new_skill_spec

    name = sg.target_name.strip() or (spec.name if spec else "")
    purpose = sg.description.strip() or (spec.description if spec else "")
    signature = opp.tool_chain_signature if opp else ""

    if spec is not None:
        proposal = proposal_from_new_skill_spec(
            spec,
            name=name,
            purpose=purpose,
        ...
        tools = [t.strip() for t in signature.split("->") if t.strip()] if signature else []
        steps = (
            [ln.strip() for ln in sg.compile_instruction.splitlines() if ln.strip()][:12]
            if sg.compile_instruction.strip()
            else ([f"Execute chain: {signature}"] if signature else [])
        )
        proposal = SkillProposal(
            name=name,
            purpose=purpose or name,
            trigger_condition=purpose,
            inputs=["Workflow input"],
            outputs=["Workflow output"],
            required_tools=tools,
            steps=steps,
            success_criteria=["Workflow completes without unhandled errors"],
            failure_modes=["Tool failure", "Invalid input", "Timeout"],
        )

    if sg.compile_instruction.strip() and opp and opp.decision == "tool_chain_composition":
        extra = sg.compile_instruction.strip()
        if extra not in proposal.steps:
            proposal = SkillProposal(
                **{
                    **proposal.to_dict(),
                    "steps": [*proposal.steps, f"Gateway detail: {extra}"],
                }
            )
    return proposal


def should_stage_skill(decision: GatewayDecision) -> tuple[bool, bool]:
    """
    是否应从网关决策进入提案驱动 staging.

    返回 (should_stage, minimal_draft).
    minimal_draft=True 表示 new_skill_proposal — 先扩最小草案再生成.
    """
    opp = decision.skill_opportunity
    if opp is not None:
        if opp.decision == "tool_chain_composition" and is_skill_execution_ready(decision):
            return True, False
        if opp.decision == "new_skill_proposal":
            return True, True
        if opp.decision == "reuse_existing_skill":
            return False, False

    if is_skill_execution_ready(decision):
        return True, False

    sg = decision.skill_generation
    if sg.action in ("merge", "upgrade") and sg.compile_instruction.strip():
        return True, False
    return False, False


def _generate_skill_core(
    target_name: str,
    compile_instruction: str,
    *,
    ...
    logger.info(
        "Quality gate for %s: validate=%s compile=%s (passed=%s, draft_mode=%s)",
        result.target_name, valid, compiled, result.passed, draft_mode,
    )
    return result
...
def generate_skill_from_proposal(
    proposal: SkillProposal,
    *,
    ...
    """
    提案驱动入口: proposal -> compile_instruction -> 复用 staging 流水线.

    minimal_draft=True (new_skill_proposal): 先 expand_minimal_proposal,
    再生成 compile_instruction, 然后走 scaffold / executor / validate.
    """
    staging_dir = Path(staging_dir)
    if client is None:
        client = OpenAIClient()

    working = expand_minimal_proposal(proposal) if minimal_draft else proposal
    if not working.normalized_name():
        raise ExecutorBridgeError("SkillProposal.name is required.")

    compile_instruction = compile_instruction_from_proposal(
        working, draft_mode=minimal_draft
    )
    return _generate_skill_core(
        working.normalized_name(),
        compile_instruction,
        proposal=working,
        client=client,
        staging_dir=staging_dir,
        overwrite=overwrite,
        draft_mode=minimal_draft,
    )
