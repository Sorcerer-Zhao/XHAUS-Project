...
def _format_skill_opportunity(decision: GatewayDecision) -> str | None:
    opp = decision.skill_opportunity
    if not opp or opp.decision == "do_nothing":
        return None
    if opp.decision == "reuse_existing_skill":
        names = ", ".join(opp.matched_skills) or "(未命名)"
        return f"♻️ 复用已有 Skill: {names} — {opp.decision_reason or '无需新建'}"
    if opp.decision == "new_skill_proposal":
        spec = opp.new_skill_spec
        name = (spec.name if spec else "") or decision.skill_generation.target_name or "(未命名)"
        gaps = ", ".join(opp.missing_capabilities) if opp.missing_capabilities else ""
        gap_note = f" 缺口: {gaps}" if gaps else ""
        return f"💡 Skill 最小提案 (未进执行层): 【{name}】{gap_note} — {opp.decision_reason or '待用户确认'}"
    if opp.decision == "tool_chain_composition":
        sig = opp.tool_chain_signature or "->".join(
            (opp.new_skill_spec.tool_chain if opp.new_skill_spec else [])
        )
        return f"🔗 工具链封装: {sig} — {opp.decision_reason or '稳定重复'}"
    return None
...
def _print_report(
    ...
    if staging is not None:
        opp = decision.skill_opportunity
        if opp and opp.decision == "new_skill_proposal":
            label = "💡 最小提案草案 — 已生成可审阅沙盒"
        elif sg.action == "merge":
            label = "🤖 高置信度 — 拟合并生成技能"
        elif sg.action == "upgrade":
            label = "🤖 高置信度 — 拟升级技能"
        else:
            label = "🤖 拟生成技能"
        lines.append(f"{label}：【{staging.target_name}】")
        ...
        opp_line = _format_skill_opportunity(decision)
        if opp_line:
            lines.append(opp_line)
            spec = decision.skill_opportunity.new_skill_spec if decision.skill_opportunity else None
            if spec and spec.rationale:
                lines.append(f"提案理由：{spec.rationale}")
        else:
...
def _interaction_loop(
    ...
    *,
    ...
    sg = decision.skill_generation
    has_executable_skill = staging is not None
    has_skill = has_executable_skill
    ...
    while True:
        ...
        if upper == "Y":
            ...
            if has_executable_skill and current is not None:
    ...
    *,
    ...
    try:
        sniff_kwargs: dict = {
            ...
            # 与工作区 skills/、IDENTITY.md 同级, 供能力缺口 + 自动 retrieve_query
            "workspace_root": identity_path.parent,
        }
    ...
    logger.info(
        "Sniffed %d row(s), %d cleaned; retrieve_query=%r recalled %d chunk(s).",
        result.fetched,
        len(result.cleaned),
        (result.retrieve_query[:60] + "...") if len(result.retrieve_query) > 60 else result.retrieve_query,
    ...
    # 注入 workspace 能力上下文 (TOOLS.md API keys + skills/ 清单 + 缺口预判)
    capability_ctx = sniff_capabilities()
    if capability_ctx:
        logger.info("Injected workspace capability context (%d chars).", len(capability_ctx))
        full_context = catalog + "\n\n# Workspace 能力清单与缺口预判\n" + capability_ctx if catalog else capability_ctx
    else:
        full_context = catalog

    # catalog 含中文触发词, Gateway 端点需 ASCII-safe; capability_ctx 已用英文名
    if full_context:
        full_context = full_context.encode("ascii", errors="replace").decode("ascii")

    _trait_tags = frozenset({"explicit_instruction", "preference"})
    trait_chunks = [
        c for c in result.chunks
        if any(t in _trait_tags for t in c.get("tags", []))
    ]
    trait_memory = serialize_chunks(trait_chunks) if trait_chunks else None
    retrieved_memory = result.retrieved_json if result.retrieved_chunks else None

    try:
        decision = evaluate_habits(
            result.json,
            client=gateway_client,
            extra_user_context=full_context,
    ...
    opp = decision.skill_opportunity
    opp_kind = opp.decision if opp else None
    logger.info(
        "Decision: opportunity=%s skill=%s identity=%s",
        opp_kind, sg.action, iu.action,
    )
    stage_skill, minimal_draft = should_stage_skill(decision)
    if sg.action == "ignore" and iu.action == "ignore" and not stage_skill:
        logger.info("No actionable skill or identity signal. Silent exit.")
    ...
    if stage_skill:
        proposal = proposal_from_gateway_decision(decision)
        logger.info(
            "Staging skill from proposal name=%s minimal_draft=%s opportunity=%s",
            proposal.normalized_name(), minimal_draft, opp_kind,
        )
        staging_result = generate_skill_from_proposal(
            proposal,
            client=forge_client,
            staging_dir=staging_dir,
            overwrite=True,
    ...
    elif is_skill_execution_ready(decision):
