...
from gateway_router import (
    ...
    evaluate_habits,
    infer_default_skill_opportunity,
    is_skill_execution_ready,
    make_llm_client,
    reconcile_gateway_decision,
)
...
class MockLLMClient:
    ...
    def complete(self, *, system: str, user: str) -> str:
            ...
            payload = {
                "skill_opportunity": {
                    ...
                    "new_skill_spec": {
                        ...
                        "description": "抓取任意 URL 并自动提取关键词, 一步完成.",
                        "proposed_triggers": ["抓网页", "提取关键词"],
                        "tool_chain": ["fetch_url", "extract_keywords"],
                        "rationale": "同一工具链在日志中稳定重复出现.",
                    },
                    "decision_reason": "fetch_url->extract_keywords repeated >=3 times",
                },
                "skill_generation": {
                    "action": "merge",
        ...
        if "视频伪造" in user or "学术语气" in user or "deepfake" in user.lower():
            payload = {
                "skill_opportunity": {
                    ...
                    "new_skill_spec": None,
                    "decision_reason": "no skill signal; identity traits only",
                },
                "skill_generation": {
                    "action": "ignore",
                    "target_name": "",
        ...
        if "reuse-existing-skill" in user:
            payload = {
                "skill_opportunity": {
                    "decision": "reuse_existing_skill",
                    "matched_skills": ["web-keyword-extractor"],
                    "missing_capabilities": [],
                    "tool_chain_signature": "fetch_url->extract_keywords",
                    "decision_reason": "existing skill fully covers the chain",
                },
                "skill_generation": {
                    "action": "merge",
                    "target_name": "should-not-create",
                    "description": "should be cleared",
                    "compile_instruction": "should be stripped",
                },
                "identity_update": {"action": "ignore", "patches": []},
            }
            return json.dumps(payload, ensure_ascii=False)

        if "new-skill-proposal" in user:
            payload = {
                "skill_opportunity": {
                    ...
                    "new_skill_spec": {
                        ...
                        "description": "封装高德路线规划 API.",
                        "proposed_triggers": ["路线规划"],
                        "tool_chain": ["amap_route"],
                        "rationale": "TOOLS.md has key but no skill yet",
                    },
                    "decision_reason": "capability gap without stable repetition",
                },
                "skill_generation": {
                    "action": "suggest",
                    "target_name": "amap-route-planner",
                    "description": "封装高德路线规划 API.",
                    "compile_instruction": "SHOULD BE STRIPPED BY RECONCILE",
                },
                "identity_update": {"action": "ignore", "patches": []},
            }
            return json.dumps(payload, ensure_ascii=False)
...
def test_case_a(client_factory) -> None:
    ...
    assert decision.skill_opportunity is not None
    assert decision.skill_opportunity.decision == "tool_chain_composition"
    ...
    assert is_skill_execution_ready(decision)
...
def test_reuse_existing_skill_reconcile() -> None:
    decision = evaluate_habits(
        "reuse-existing-skill trigger",
        client=MockLLMClient(),
        max_retries=1,
        retry_backoff=0.0,
    )
    assert decision.skill_opportunity is not None
    assert decision.skill_opportunity.decision == "reuse_existing_skill"
    assert decision.skill_generation.action == "ignore"
    assert not decision.skill_generation.compile_instruction.strip()
    assert not is_skill_execution_ready(decision)


def test_new_skill_proposal_not_execution_ready() -> None:
    decision = evaluate_habits(
        "new-skill-proposal trigger",
        client=MockLLMClient(),
        max_retries=1,
        retry_backoff=0.0,
    )
    assert decision.skill_opportunity is not None
    assert decision.skill_opportunity.decision == "new_skill_proposal"
    assert decision.skill_generation.action == "suggest"
    assert not decision.skill_generation.compile_instruction.strip()
    assert decision.skill_generation.new_skill_spec is not None
    assert not is_skill_execution_ready(decision)


def test_infer_default_skill_opportunity_backcompat() -> None:
    sg = SkillGeneration(
        action="merge",
        target_name="foo-bar",
        description="d",
        compile_instruction="do it",
    )
    opp = infer_default_skill_opportunity(sg)
    assert opp.decision == "tool_chain_composition"
    decision = reconcile_gateway_decision(
        GatewayDecision(
            skill_generation=sg,
            identity_update={"action": "ignore", "patches": []},
...
def test_evaluate_habits_injects_memory_sections() -> None:
    client = MockLLMClient()

    class _Capturing(MockLLMClient):
        last_user = ""
    ...
    evaluate_habits(
        ...
        max_retries=1,
        retry_backoff=0.0,
        retrieved_memory='[{"chunk_id":"c1"}]',
        trait_memory='[{"chunk_id":"t1"}]',
        capability_snapshot={"gaps": [{"name": "amap"}]},
    )
    assert "<RETRIEVED_MEMORY>" in cap.last_user
    assert "<TRAIT_MEMORY>" in cap.last_user
    assert "<CAPABILITY_SNAPSHOT>" in cap.last_user
    assert "amap" in cap.last_user
...
def main(argv: list[str] | None = None) -> int:
    ...
    for name, fn in [
        ...
        ("reuse existing skill", test_reuse_existing_skill_reconcile),
        ("new skill proposal", test_new_skill_proposal_not_execution_ready),
        ("infer default opportunity", test_infer_default_skill_opportunity_backcompat),
        ("memory context sections", test_evaluate_habits_injects_memory_sections),
